...
def clarke_wright_route(nodes_ids, start_id, G):
    ...
    return final_route
print("6-th step done")

# Генерация обучающих примеров (адаптировано под CW)
pick_nodes = list({c['nearest_node'] for c in containers})
start_node = pick_nodes[0]
route = clarke_wright_route(pick_nodes, start_node, G_road)

train_examples = []
remaining = set(route[1:-1])  # Исключаем депо из оставшихся узлов
current = start_node

for next_node in route[1:]:  # Пропускаем начальное депо
    ...
    if next_node in remaining:
        remaining.remove(next_node)
    current = next_node
print("7-th step done")

# 7-10. Остальной код без изменений
class RouteDataset(Dataset):
    def __init__(self, examples):
        self.examples = examples
    def __len__(self):
